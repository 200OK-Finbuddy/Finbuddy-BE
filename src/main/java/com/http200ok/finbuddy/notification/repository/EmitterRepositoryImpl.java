...
public class EmitterRepositoryImpl implements EmitterRepository {
    ...
    @Override
    public void deleteAllEmitterStartWithMemberId(String memberId) { // 해당 회원과 관련된 모든 emitter를 지움
        emitters.forEach(
                (key, emitter) -> {
    ...
    @Override
    public void deleteAllEventCacheStartWithMemberId(String memberId) { // 해당 회원과 관련된 모든 이벤트를 지움
        eventCache.forEach(
                (key, emitter) -> {
                    if (key.startsWith(memberId)) {
